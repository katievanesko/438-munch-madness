...
//    func retrieveVenues(location : String, category: String, limit : Int, sortBy: String, price: String, completionHandler: @escaping ([Restaurant]?, Error?) -> Void){
    func retrieveVenues(location : String, category: String, limit : Int, sortBy: String, price: String, radius:Int, completionHandler: @escaping ([Restaurant]?, Error?) -> Void) -> Void{
        ...
        //Create URL
        let baseURL = "https://api.yelp.com/v3/businesses/search?location=\(location)&categories=\(category)&limit=\(limit)&sort_by?=\(sortBy)&radius=\(radius)&price=\(price)"
